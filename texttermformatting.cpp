                         ...
                         QTextCharFormat const & defaultTextCharFormat)
{
    ...
    case 38 : { // http:/read/en.m.wikipedia.org/wiki/ANSI_escape_code http://misc.flogisoft.com/bash/tip_colors_and_formatting http://invisible-island.net/xterm/ctlseqs/ctlseqs.html
        if (i.hasNext()) {
            bool ok = false;
    ...
    case 48 : { // http://en.m.wikipedia.org/wiki/ANSI_escape_code http://misc.flogisoft.com/bash/tip_colors_and_formatting http://invisible-island.net/xterm/ctlseqs/ctlseqs.html
        if (i.hasNext()) {
            bool ok = false;
            int selector = i.next().toInt(&ok);
            Q_ASSERT(ok);
...
QTextCursor
setTextTermFormatting(QTextDocument * document, QString const & text)
{
    QRegExp const & escapeSequenceExpression = Settings::getPersistentSettings().escapeSequenceExpression;
